...
def prepro(input):
    ...
    #print(input.shape) --> (210, 160, 3)
    input = input[35:195]  # crop --> (160, 160, 3)
    input = input[::2, ::2, 0]  # downsample by factor of 2 --> (80, 80)
    ...
    input = input.astype(np.float).ravel()
    #print(input.shape) --> (6400,)
    return input
...
def policy_forward(x):
    h = np.dot(model['W1'], x)  # (200 X 6400) dot (6400,) => (200,)
    h[h < 0] = 0  # ReLU nonlinearity
    logp = np.dot(model['W2'], h)  # (200,) dot (200,) ==> ()
    p = sigmoid(logp) # [0, 1] 사이의 확률이 되도록 함
