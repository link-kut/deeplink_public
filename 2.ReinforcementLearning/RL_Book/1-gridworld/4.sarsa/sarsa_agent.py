...
class SARSAgent:
    def __init__(self, actions):
        ...
        self.epsilon = 0.1
        self.q_table = defaultdict(lambda: [0.0, 0.0, 0.0, 0.0]) # 키값이 없으면 lambda 함수에 의해 [0.0, 0.0, 0.0, 0.0]을 값으로 반환
        self.samples_for_print = []

    # append sample to memory(state, reward, done)
    def save_samples_for_print(self, state, action):
        self.samples_for_print.append([state, action])
