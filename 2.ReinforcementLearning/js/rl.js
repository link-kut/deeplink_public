...
(function(global) {
  ...
  // p = [0.1, 0.4, 0.4, 0.1] - Uniform Distribution 에 따르는 Random Action 추출
  ...
  var TDAgent = function(env, opt) {
    this.update = getopt(opt, 'update', 'sarsa'); // qlearn | sarsa
    this.gamma = getopt(opt, 'gamma', 0.75); // future reward discount factor
    this.epsilon = getopt(opt, 'epsilon', 0.2); // for epsilon-greedy policy
    this.alpha = getopt(opt, 'alpha', 0.01); // value function learning rate
    this.eta = getopt(opt, 'eta', 0.02); // epsilon decay rate
  ...
  TDAgent.prototype = {
    ...
    learn: function(r1) {
      ...
      if (!(this.r0 == null)) {
        this.learnFromTuple(this.s0, this.a0, this.r0, this.s1, this.a1);
      }
